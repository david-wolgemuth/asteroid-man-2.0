...
(0, _jquery2.default)(document).ready(function () {
    ...
    (0, _jquery2.default)(".start-hidden").hide();
    ...
    // Player
    var FALLING_IMAGE_SWITCH = 0.1,
        PLAYER_W = 20,
        PLAYER_H = 40;

    // Item Sizes and Ratios
    var MIN_PLATFORM_HEIGHT = 80,
    ...
    var Loadbar = {
        ...
        onload: function onload() {
            ...
            (0, _jquery2.default)("#loading-bar").css("width", percent + "%");
            if (percent == 100) {
                (0, _jquery2.default)("#loading-bar-outline").hide();
                (0, _jquery2.default)("#audio-options").show();
                (0, _jquery2.default)("#audio-on").click(function () {
                    Sounds.muted = false;
                    (0, _jquery2.default)("#audio-options").hide();
                    MenuBar.init();
                });
                (0, _jquery2.default)("#audio-off").click(function () {
                    (0, _jquery2.default)("#audio-options").hide();
                    MenuBar.init();
                });
            }
    ...
    var Images = {
        ...
        load: function load(destination, path, length) {
            for (var i = 0; i < length; i++) {
                var image = new Image();
                var path_i = 'images/' + path + '/' + i + '.png';
                image.src = path_i;
        ...
        init: function init() {
            var destinations = [[Images.starfield, 'starfield', 1], [Images.falling.left, 'falling-left', 15], [Images.falling.right, 'falling-right', 15], [Images.jumping.left, 'jumping-left', 9], [Images.jumping.right, 'jumping-right', 9], [Images.jetPack, 'jet-pack', 10], [Images.platform, 'platform', 14], [Images.coin, 'coin', 6], [Images.fuel, 'fuel', 1], [Images.asteroid, 'asteroid', 1], [Images.explosion, 'explosion', 6], [Images.comet, 'comet', 5]];
    ...
    var Sounds = {
        ...
        main_menu: [],
        jump: [],
        load: function load(destination, path, x, volume, loop) {
            for (var i = 0; i < x; i++) {
        ...
        webAudio: function webAudio() {
             ...
             */
            WebAudioAPISoundManager = function WebAudioAPISoundManager(context) {
                ...
                stopSoundWithUrl: function stopSoundWithUrl(url) {
                    if (this.playingSounds.hasOwnProperty(url)) {
                        for (var i in this.playingSounds[url]) {
                            if (this.playingSounds[url].hasOwnProperty(i)) this.playingSounds[url][i].stop(0);
                        }
                    }
                }
            };
             ...
             */
            WebAudioAPISound = function WebAudioAPISound(url, options) {
                this.settings = {
                    loop: false
                };
            ...
            WebAudioAPISound.prototype = {
                play: function play() {
                    ...
                    if (typeof buffer !== "undefined") {
                        ...
                        if (!this.manager.playingSounds.hasOwnProperty(this.url)) this.manager.playingSounds[this.url] = [];
        ...
        init: function init() {
            Sounds.webAudio();
            var destinations = [[Sounds.theme, 'theme', 1, 0.2, true], [Sounds.fuel, 'fuel', 1, 1], [Sounds.coin, 'coin', 11, 1], [Sounds.asteroid, 'asteroid', 2, 0.5], [Sounds.comet, 'comet', 1, 0.5], [Sounds.credits, 'credits', 2, 0.7], [Sounds.empty_jetPack, 'empty-jetpack', 2, 1], [Sounds.life, 'life', 1, 1], [Sounds.go, 'go', 1, 1], [Sounds.main_menu, 'main-menu', 1, 1], [Sounds.jetPack, 'jet-pack', 1, 0.4], [Sounds.jump, 'jump', 3, 1.5], [Sounds.game_over, 'game-over', 1, 1]];
            for (var i = 0; i < destinations.length; i++) {
                var d = destinations[i];
    ...
    var Random = {
        ...
        bool: function bool(max) {
            return Random.range(0, max) === 0;
        },
        send: {
            coin: function coin() {
                return Random.bool(COIN_FREQUENCY);
            },
            platform: function platform() {
                return Random.bool(PLATFORM_FREQUENCY);
            },
            asteroid: function asteroid() {
                return Random.bool(Session.game.asteroidFrequency);
            },
            fuel: function fuel() {
                return Random.bool(FUEL_FREQUENCY);
            },
            life: function life() {
                return Random.bool(XLIFE_FREQUENCY);
            },
            comet: function comet() {
                return Random.bool(COMET_FREQUENCY);
            }
        },
        r: {
            platform: function platform() {
                return Random.range(MIN_PLATFORM_HEIGHT, MAX_PLATFORM_HEIGHT);
            },
            asteroid: function asteroid() {
                return Random.range(MIN_ASTEROID_R, MAX_ASTEROID_R);
            },
            comet: function comet() {
                return Random.range(MIN_COMET_R, MAX_COMET_R);
            }
        },
        x: {
            platform: function platform() {
                var column = Random.range(1, Math.floor(WIDTH / POSITION_SPACE - 4)); // I have no idea why -4 works here
            ...
            fuel: function fuel() {
                return Random.range(FUEL_W, WIDTH - FUEL_W);
            },
            comet: function comet() {
                return Random.range(0, WIDTH);
            }
        ...
        image: {
            platform: function platform() {
                return Random.range(0, Images.platform.length - 1);
            }
        }
    };

    var Collision = {
        rectRect: function rectRect(rectA, rectB) {
            return !(rectB.x > rectA.x + rectA.w || rectB.x + rectB.w < rectA.x || rectB.y > rectA.y + rectA.h || rectB.y + rectB.h < rectA.y);
        },
        rectCircle: function rectCircle(rect, circle) {
            ...
            var cornerDistSq = Math.pow(distX - rect.w / 2, 2) + Math.pow(distY - rect.h / 2, 2);
    ...
    var Draw = {
        lineWidth: 2,
        init: function init() {
            Draw.ctx = (0, _jquery2.default)("#canvas")[0].getContext('2d');
        },
        clear: function clear() {
            Draw.ctx.clearRect(0, 0, WIDTH, HEIGHT);
        },
        image: function image(_image, x, y, w, h) {
            Draw.ctx.drawImage(_image, x, y, w, h);
        },
        rotatedImage: function rotatedImage(image, x, y, w, h, degree) {
            Draw.ctx.save();
        ...
        rect: function rect(x, y, w, h, fill) {
            if (fill) {
                Draw.ctx.beginPath();
        ...
        circle: function circle(x, y, r, fill) {
            Draw.ctx.beginPath();
        ...
        score: function score(_score) {
            ...
            Draw.text(40, 31, _score);
        },
        remainingLives: function remainingLives(lives) {
            for (var i = 1; i <= lives; i++) {
                var x = WIDTH - i * 30;
                Draw.ctx.drawImage(Images.falling.right[0], x, 5, PLAYER_W * 1.5, PLAYER_H);
            }
        },
        remainingFuel: function remainingFuel(fuel) {
    ...
    var MenuBar = {
        ...
        init: function init() {
            ...
            (0, _jquery2.default)("#menu, #menu *").show();
            (0, _jquery2.default)("#show-tutorial").click(function () {
                MenuBar.tutorial();
            });
            (0, _jquery2.default)("#show-credits").click(function () {
                MenuBar.credits();
            });
            (0, _jquery2.default)("#play").click(function () {
                MenuBar.play();
            });
            (0, _jquery2.default)("#return").click(function () {
                Sounds.coin.random();
                (0, _jquery2.default)("#credits").hide();
                (0, _jquery2.default)("#menu").show();
            });
            (0, _jquery2.default)("#return-main-menu").click(function () {
                Sounds.credits[0].play();
                Sounds.theme[0].stop();
                (0, _jquery2.default)("#play-again").hide();
                (0, _jquery2.default)("#return-main-menu").hide();
                (0, _jquery2.default)("#canvas").hide();
                (0, _jquery2.default)("body").css("background-image", "url('images/starfield/0.png')");
                (0, _jquery2.default)("body").css("background-size", "100% 100%");
                window.cancelAnimationFrame(Session.animation);
                (0, _jquery2.default)("#menu").show();
            });
            (0, _jquery2.default)("#play-again").click(function () {
                (0, _jquery2.default)("#play-again").hide();
                (0, _jquery2.default)("#return-main-menu").hide();
                window.cancelAnimationFrame(Session.animation);
                Session.init();
            });
            (0, _jquery2.default)("#continue").click(function () {
                (0, _jquery2.default)("#tut-" + MenuBar.continueI).hide();
                MenuBar.continueI++;
                Sounds.coin[MenuBar.continueI].play();
                if (MenuBar.continueI == 7) {
                    (0, _jquery2.default)("#tutorial").hide();
                    (0, _jquery2.default)("#menu").show();
                } else {
                    (0, _jquery2.default)("#tut-" + MenuBar.continueI).show();
                }
            });
        },
        play: function play() {
            (0, _jquery2.default)("#menu").hide();
            (0, _jquery2.default)("body").css("background", "#AAA");
            (0, _jquery2.default)("#canvas").show();
            Session.init();
        },
        tutorial: function tutorial() {
            ...
            Sounds.coin[0].play();
            (0, _jquery2.default)("#menu").hide();
            (0, _jquery2.default)("#tutorial").show();
            (0, _jquery2.default)("#tut-" + MenuBar.continueI).show();
        },
        credits: function credits() {
            Sounds.credits[1].play();
            (0, _jquery2.default)("#menu").hide();
            (0, _jquery2.default)("#credits").show();
        }
    };
    ...
    var Session = {
        ...
        init: function init() {
            ...
            Sounds.theme[0].play();
            Sounds.life[0].play();
            Session.canvas = (0, _jquery2.default)("#canvas")[0];
            ...
            (0, _jquery2.default)("#ready").show();
            Session.loop();
        },
        checkText: function checkText() {
            if ((0, _jquery2.default)("#ready").is(":visible")) {
                (0, _jquery2.default)("#ready").hide();
                (0, _jquery2.default)("#go").show();
                Sounds.go[0].play();
                Session.game.running = true;
                return true;
            } else if ((0, _jquery2.default)("#game-over").is(":visible")) {
                (0, _jquery2.default)("#game-over").hide();
                (0, _jquery2.default)("#play-again").show();
                (0, _jquery2.default)("#return-main-menu").show();
            }
        },
    ...
    function Game() {
        ...
        this.explosions = [];
        this.comets = [];
        this.allObjects = [this.platforms, this.fuel, this.lives, this.coins, this.asteroids, this.debree, this.explosions, this.comets];
        this.holdGo = 100;
        this.events = [];
        ...
        this.holdGoFunction = function () {
            if ((0, _jquery2.default)("#go").is(":visible")) {
                if (this.holdGo > 0) {
                    this.holdGo -= 1;
                } else {
                    (0, _jquery2.default)("#go").fadeOut(1000);
                }
            }
        };
        ...
        this.gameOver = function () {
            ...
            (0, _jquery2.default)("#game-over").show();
            this.running = false;
        };
    ...
    function Player() {
        ...
        this.draw = function () {
            ...
            if (this.jumping === true) {
                ...
                if (this.direction === RIGHT) img = Images.jumping.right[Math.floor(this.imageI)];else if (this.direction === LEFT) img = Images.jumping.left[Math.floor(this.imageI)];
            } else {
                ...
                if (this.direction === RIGHT) img = Images.falling.right[Math.floor(this.imageI)];else if (this.direction === LEFT) img = Images.falling.left[Math.floor(this.imageI)];
            }
            Draw.image(img, this.x, this.y, this.w * 1.5, this.h);
            if (this.jetPackI >= 0) {
                var jet = Images.jetPack[Math.floor(this.jetPackI)];
                if (this.direction === RIGHT) {
                    Draw.image(jet, this.x - JETPACK_R, this.y + this.h / 2, JETPACK_R, JETPACK_R);
                } else if (this.direction === LEFT) {
                    Draw.image(jet, this.x + this.w * 1.5, this.y + this.h / 2, JETPACK_R, JETPACK_R);
                }
            }
        };
    ...
    function Sprite() {
        ...
        this.asteroid = function (x, w) {
            ...
            this.draw = function () {
                ...
                Draw.rotatedImage(Images.asteroid[0], this.x - c, this.y - c, this.w + c * 2, this.h + c * 2, Math.floor(this.angle));
            };
        };
        ...
        this.comet = function (x, y, w) {
            ...
            this.collision = function () {
                this.imageI = 6;
                if (this.arg != HIT_PLAYER) Sounds.comet.random();
                this.arg = HIT_PLAYER;
            };
            this.specialUpdate = function () {
            ...
            this.draw = function () {
                ...
                if (this.imageI < 5) Draw.rotatedImage(Images.comet[Math.floor(this.imageI)], this.x - this.w * 1.5, this.y + this.h / 2, this.w * 1.8 * 2.2, this.h * 2.2, degree);else Draw.rotatedImage(Images.explosion[Math.floor(this.imageI - 6)], this.x - this.w / 2, this.y, this.w * 2.2, this.h * 2.2, degree);
